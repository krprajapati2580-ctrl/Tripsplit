...
import { Plus, List, ArrowLeft, Users, DollarSign, Check, Trash2, CheckCircle, HelpCircle, Camera, Lock, Pencil } from "lucide-react";
...
export function DashboardScreen({
  ...
  return (
    <div className="flex flex-col h-full bg-[#f8fafc]">
      ...
      <div className="bg-slate-50 text-slate-900 px-4 pt-4 pb-4 rounded-b-2xl shadow-2xs border-b border-slate-200/60 flex items-center justify-between">
        <div>
          <h1 className="text-lg font-bold font-sans tracking-tight text-slate-900">Trip Expenses</h1>
          <p className="text-[10px] text-slate-500 font-sans mt-0.5">
            {expenses.length} expense{expenses.length !== 1 && "s"} total
          </p>
        </div>
        <div className="flex items-center gap-1.5 bg-blue-50 border border-blue-100 px-2.5 py-1 rounded-full text-[10px] font-bold text-blue-600">
          <Users size={12} />
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3 pb-24">
        ...
        {offlineQueue && offlineQueue.length > 0 && (
          <div className="bg-amber-50/70 border border-amber-200/60 rounded-2xl p-4 space-y-3 mb-2 shadow-2xs">
            <div className="flex items-center justify-between">
              ...
              <span className="text-[9px] bg-amber-100 text-amber-700 px-2 py-0.5 rounded font-bold uppercase tracking-wider">
                WorkManager Sync
              </span>
            </div>
            ...
            <div className="space-y-2">
              {offlineQueue.map((item) => (
                <div key={item.id} className="bg-white border border-amber-100 rounded-xl p-3 flex justify-between items-center text-xs shadow-3xs">
                  ...
                  <div className="shrink-0 flex items-center gap-2">
                    ...
                    {onRemoveQueuedReceipt && (
                      <button
                        ...
                        className="text-slate-300 hover:text-red-500 p-1 rounded-lg hover:bg-red-50/50 transition-colors cursor-pointer shrink-0"
            ...
            {[...expenses].reverse().map((expense, idx) => {
              ...
              return (
                <motion.div
                  ...
                  className="bg-white rounded-2xl p-4 shadow-2xs border border-slate-100 hover:border-blue-200 transition-colors flex justify-between items-center relative group"
                  id={`expense-card-${expense.id}`}
                >
                  <div className="flex-1 pr-4 min-w-0">
                    <h4 className="font-bold text-slate-950 text-sm leading-snug">{expense.description}</h4>
                    <div className="flex flex-wrap items-center gap-x-2 gap-y-0.5 mt-1 text-[11px] text-slate-500">
                      <span>Paid by <strong className="text-slate-800 font-semibold">{payerName}</strong></span>
                    ...
                    {expense.items && expense.items.length > 0 && (
                      <div className="mt-2 text-[10px] text-slate-500 bg-slate-100/50 p-1.5 rounded-lg border border-slate-200/50 max-w-sm">
                        <div className="font-bold text-slate-700 mb-0.5 flex items-center gap-1">
                          <span className="text-[8px] bg-blue-100 text-blue-700 px-1 py-0.5 rounded font-bold uppercase tracking-wider">Itemized Receipt</span>
                        </div>
                        <div className="space-y-0.5 text-[9px] text-slate-500">
                          {expense.items.filter(i => i.type === "FOOD").slice(0, 3).map((item, idx) => (
                            <div key={idx} className="flex justify-between gap-4">
                              ...
                              <span className="font-mono font-bold text-slate-600 shrink-0">{formatCurrency(item.price, currencySymbol)}</span>
                            </div>
                          ))}
                  ...
                  <div className="flex flex-col items-end gap-2 shrink-0 text-right max-w-[45%] pr-3 md:pr-5">
                    <span className="text-xs sm:text-sm md:text-base font-extrabold text-slate-900 font-mono break-all leading-tight">
                      {formatCurrency(expense.totalAmount, currencySymbol)}
                    </span>
                    
                    <div className="flex items-center gap-1 bg-slate-50 border border-slate-100 rounded-lg p-0.5 shadow-3xs">
                      {onStartEdit && (
                        <button
                          id={`edit-btn-${expense.id}`}
                          onClick={() => onStartEdit(expense)}
                          className="text-slate-400 hover:text-blue-600 p-1 rounded-md hover:bg-white transition-all"
                          title="Edit expense"
                        >
                          <Pencil size={12} />
                        </button>
                      ...
                      {onDeleteExpense && (
                        <button
                          ...
                          className="text-slate-400 hover:text-red-600 p-1 rounded-md hover:bg-white transition-all"
...
// BALANCES SCREEN
// ---------------------------------------------------------
export function BalancesScreen({ expenses, users, currencySymbol = "₹" }: ScreenProps) {
  ...
  return (
    <div className="flex flex-col h-full bg-[#f8fafc]">
      ...
      <div className="bg-slate-50 text-slate-900 px-4 pt-4 pb-4 rounded-b-2xl shadow-2xs border-b border-slate-200/60 flex items-center justify-between">
        <div>
          <h1 className="text-lg font-bold font-sans tracking-tight text-slate-900">Trip Balances</h1>
          <p className="text-[10px] text-slate-500 font-sans mt-0.5">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-5 pb-24">
        ...
        <div>
          <h3 className="text-[10px] font-bold uppercase tracking-[0.15em] text-slate-400 px-1 mb-2">
          ...
          <div className="space-y-2">
            {users.map((user) => {
              ...
              return (
                <div
                  key={user.id}
                  className="bg-white rounded-2xl p-4 shadow-2xs border border-slate-100 flex justify-between items-center"
                  id={`balance-row-${user.id}`}
                >
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-xl bg-slate-50 text-slate-700 border border-slate-200 font-bold text-xs flex items-center justify-center">
        ...
        <div>
          <div className="flex items-center justify-between mb-2 px-1">
            ...
            <span className="text-[9px] bg-slate-100 text-slate-600 border border-slate-200/60 px-1.5 py-0.5 rounded font-bold">
              Minimized
            </span>
          </div>

          {debts.length === 0 ? (
            <div className="bg-white rounded-2xl p-6 border border-dashed border-slate-200 flex flex-col items-center text-center">
              <div className="w-9 h-9 bg-emerald-50 text-emerald-500 rounded-full flex items-center justify-center mb-2">
                <CheckCircle size={18} />
              </div>
              <p className="text-xs font-bold text-slate-800">All settled up!</p>
              <p className="text-[10px] text-slate-400 mt-0.5">No transfers are required right now.</p>
            </div>
          ) : (
            <div className="space-y-2">
              {debts.map((debt, idx) => {
                return (
                  <div
                    key={idx}
                    className="bg-white rounded-2xl p-4 border border-slate-200/80 flex justify-between items-center hover:border-blue-200 transition-colors"
                    id={`debt-card-${idx}`}
                  >
                    <div>
                      <div className="flex items-center gap-1.5 text-xs font-bold text-slate-900">
                        <span>{debt.fromName}</span>
                        <span className="text-[10px] font-normal text-slate-400">pays</span>
                        <span>{debt.toName}</span>
                      </div>
                      <p className="text-[10px] text-slate-400 mt-0.5">Settle shared balance</p>
                    </div>
...
export function AddExpenseScreen({
  ...
  return (
    <div className="flex flex-col h-full bg-[#f8fafc] relative">
      ...
      {/* Android Top Header bar */}
      <div className="bg-slate-50 text-slate-900 px-3 pt-4 pb-4 rounded-b-2xl shadow-2xs border-b border-slate-200/60 flex items-center gap-3">
        <button
          id="back-btn"
        ...
        <div>
          <h1 className="text-base font-bold font-sans tracking-tight text-slate-900">{editingExpense ? "Edit Expense" : "Add New Expense"}</h1>
        </div>
      </div>

      <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-4 space-y-4 pb-24">
        {error && (
          <div className="bg-red-50 text-red-600 text-xs p-3 rounded-xl border border-red-200/60 font-medium">
            {error}
          </div>
        )}
        ...
        {/* Description Field */}
        <div>
          <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-[0.12em] mb-1 px-1">
            What was this for?
          </label>
          <input
            ...
            className="w-full bg-white border border-slate-200 rounded-xl px-4 py-3 text-sm focus:outline-hidden focus:border-blue-600 focus:ring-1 focus:ring-blue-600 transition-all text-slate-900 placeholder-slate-400"
          />
        </div>
          ...
          <div className="flex gap-2">
            <div className="relative flex-1">
              ...
              <input
                ...
                placeholder="0.00"
                className="w-full bg-white border border-slate-200 rounded-xl pl-8 pr-4 py-3 text-sm focus:outline-hidden focus:border-blue-600 focus:ring-1 focus:ring-blue-600 font-mono transition-all text-slate-900 placeholder-slate-400"
              />
            </div>
            <button
              type="button"
              ...
              disabled={isScanning}
              className="flex items-center justify-center bg-blue-50 text-blue-600 border border-blue-100 hover:bg-blue-100 active:bg-blue-200 p-3 rounded-xl transition-all cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed min-w-[48px]"
              title="Scan Receipt"
            >
              {isScanning ? (
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-3 space-y-3 shadow-2xs">
            <div className="flex justify-between items-center pb-2 border-b border-slate-100">
              <div className="flex items-center gap-1.5">
                <h3 className="text-xs font-bold text-slate-800">Scanned Receipt Items</h3>
                <span className="text-[9px] bg-blue-50 text-blue-600 px-1 py-0.5 rounded-sm font-semibold">
                  Proportional Tax
                </span>
              </div>
              <button 
            ...
            <div className="space-y-3 max-h-[250px] overflow-y-auto pr-1">
              ...
              {receiptItems.filter(item => item.type === "FOOD").map((item) => {
                ...
                return (
                  <div key={index} className={`p-2 rounded-lg border transition-all ${isItemChecked ? "border-slate-100 bg-slate-50/50" : "border-slate-100/50 opacity-60 bg-slate-100/20"}`}>
                    {/* Item Title & Price */}
                    <div className="flex items-center justify-between">
                      <label className="flex items-center gap-2 cursor-pointer select-none">
                        ...
                        />
                        <span className="font-bold text-xs text-slate-800 truncate max-w-[120px]">{item.name}</span>
                        <span className="text-[8px] bg-emerald-50 text-emerald-700 px-1 py-0.5 rounded-sm font-semibold uppercase">Food</span>
                      </label>
                    ...
                    {isItemChecked && (
                      <div className="mt-2 pl-6">
                        <div className="flex justify-between items-center mb-1">
                          <p className="text-[9px] font-semibold text-slate-400 uppercase tracking-wider">Shared by:</p>
                          <label className="flex items-center gap-1 cursor-pointer select-none bg-blue-50/50 hover:bg-blue-100/50 px-1.5 py-0.5 rounded border border-blue-100 transition-all">
                            <input
                              type="checkbox"
                            ...
                            />
                            <span className="text-[9px] font-bold text-blue-600 uppercase tracking-wide">Everyone</span>
                          </label>
                        </div>
                              ...
                              <button
                                ...
                                className={`px-2 py-0.5 rounded-full text-[10px] font-medium transition-all cursor-pointer ${
                                  isUserChecked 
                                    ...
                                    : "bg-slate-100 text-slate-500 border border-slate-200/50 hover:bg-slate-200/50"
                                }`}
                              >
                                {user.name}
                              </button>
              ...
              {receiptItems.filter(item => item.type === "TAX").map((item) => {
                const index = receiptItems.indexOf(item);
                return (
                  <div key={index} className="p-2 rounded-lg border border-slate-200 bg-slate-100/60 text-slate-500 flex items-center justify-between select-none opacity-80">
                    <div className="flex items-center gap-1.5">
                      <Lock className="w-3 h-3 text-slate-400" />
                      <span className="font-bold text-xs text-slate-600 truncate max-w-[130px]">{item.name}</span>
                      <span className="text-[8px] bg-slate-200 text-slate-600 px-1 py-0.5 rounded-sm font-semibold uppercase">Tax</span>
                    </div>
            ...
            <div className="pt-2 border-t border-slate-100 space-y-1.5">
              <div className="flex justify-between items-center text-xs">
                <span className="font-bold text-slate-600">Calculated Sum:</span>
              ...
              <div className="bg-blue-50/50 rounded-lg p-2.5 space-y-1 border border-blue-100/50">
                <p className="text-[9px] font-bold text-blue-500 uppercase tracking-wider">Calculated Shares (Proportional Tax Included):</p>
        ...
        <div>
          ...
          <select
            ...
            className="w-full bg-white border border-slate-200 rounded-xl px-4 py-3 text-sm focus:outline-hidden focus:border-blue-600 focus:ring-1 focus:ring-blue-600 transition-all text-slate-900 cursor-pointer"
          >
            {users.map((u) => (
        ...
        {receiptItems.length === 0 && (
          <div>
            <div className="flex justify-between items-center mb-1 px-1">
              ...
              <div className="space-x-2 text-[9px] text-blue-600 font-bold">
            ...
            <div className="bg-white border border-slate-100 rounded-xl p-2 space-y-1 shadow-2xs">
              {users.map((user) => {
                const isChecked = involvedUserIds.includes(user.id);
                return (
                  <label
                    key={user.id}
                    className="flex items-center gap-3 p-2 rounded-lg hover:bg-slate-50 cursor-pointer transition-all text-xs"
                  >
                    <input
                      type="checkbox"
                      checked={isChecked}
                      ...
                      className="w-4 h-4 rounded text-blue-600 border-slate-300 focus:ring-blue-600 cursor-pointer"
                    />
                    <span className={`font-semibold ${isChecked ? "text-slate-900" : "text-slate-400"}`}>
                      {user.name}
                    </span>
                  </label>
                );
